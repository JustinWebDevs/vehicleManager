                ...
                <form action="{{ route('ordenes.update', $orden->id) }}" method="POST">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="vehiculo_id" id="vehiculo_id" class="form-select mt-1 block w-full" required>
                            @foreach($vehiculos as $vehiculo)
                                <option value="{{ $vehiculo->id }}" {{ $vehiculo->id == $orden->vehiculo_id ? 'selected' : '' }}>
                    ...
                    <div class="mt-4">
                        ...
                        <select name="tipo_orden" id="tipo_orden" class="form-select mt-1 block w-full" required>
                            <option {{$orden->tipo_orden == 'Correctivo' ? 'selected' : ''}} value="Correctivo">Correctivo</option>
                            <option {{$orden->tipo_orden == 'Preventivo' ? 'selected' : ''}} value="Preventivo">Preventivo</option>
                        </select>
                    </div>

                    <div class="mt-4">
                        <label for="fecha" class="block text-sm font-medium text-gray-700">Fecha</label>
                        <input type="date" name="fecha" id="fecha" class="form-input mt-1 block w-full" value="{{ $orden->fecha }}" required>
                    </div>

                    <div class="mt-6">
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring focus:ring-blue-300 disabled:opacity-25 transition">Actualizar Orden</button>
                    </div>
                </form>
            </div>
        </div>
